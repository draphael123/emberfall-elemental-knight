...
import { cityRank as getCityRank, earnedResources, effectiveCardCost, incomingAttack, parseCampaignSave, powerBoost, repairCampaignSave, restorationObjectives, retreatResources, rewardBand, scaledEnemyHp, seededShuffle } from "./game-logic";
...
export default function Home() {
  ...
  const restoration = restorationObjectives({rescued,blueprint,wandererRescued,forgeLevel,sanctumLevel,hallLevel});
  const nextRestoration = restoration.find(objective=>!objective.done)?.label ?? "Fortified city complete";

  const deck = useMemo(
    () => [...BASE_CARDS, ...ELEMENT_CARDS[selected[0]], ...ELEMENT_CARDS[selected[1]]],
    [selected],
  );
  useEffect(()=>{const raw=localStorage.getItem("emberfall-save");if(!raw)return;const parsed=parseCampaignSave(raw);if(!parsed){setTownMessage("The old campaign record was damaged; a fresh ledger has been opened.");return}const data=repairCampaignSave(parsed);setRescued(!!data.rescued);setBlueprint(!!data.blueprint);setForgeLevel(data.forgeLevel as number);setGold(data.gold as number);setSupplies(data.supplies as number);setUpgraded(Array.isArray(data.upgraded)?data.upgraded as string[]:[]);setUpgradePaths(data.upgradePaths&&typeof data.upgradePaths==="object"?data.upgradePaths as Record<string,"power"|"mastery">:{});setMaraGift(!!data.maraGift);setIlyraGift(!!data.ilyraGift);setQuartermasterGift(!!data.quartermasterGift);setSanctumLevel(data.sanctumLevel as number);setSanctumBlessing(typeof data.sanctumBlessing==="string"?data.sanctumBlessing:"fire");setHallLevel(data.hallLevel as number);setDifficulty(data.difficulty as number);setCampaignWins(data.campaignWins as number);setAchievements(Array.isArray(data.achievements)?data.achievements as string[]:[]);setTownDay(data.townDay as number);setWandererRescued(!!data.wandererRescued);if(data.buildingPos&&typeof data.buildingPos==="object")setBuildingPos(data.buildingPos as typeof buildingPos)},[]);
