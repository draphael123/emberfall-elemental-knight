...
export default function Home() {
  ...
  useEffect(()=>{const saved=localStorage.getItem("emberfall-save");if(saved){try{const data=JSON.parse(saved);setRescued(!!data.rescued);setBlueprint(!!data.blueprint);setForgeLevel(data.forgeLevel||0);setGold(data.gold??45);setUpgraded(data.upgraded||[]);setMaraGift(!!data.maraGift);setSanctumLevel(data.sanctumLevel||1);setSanctumBlessing(data.sanctumBlessing||"fire");setHallLevel(data.hallLevel||1);setDifficulty(data.difficulty||0);setCampaignWins(data.campaignWins||0);setWandererRescued(!!data.wandererRescued);if(data.buildingPos)setBuildingPos(data.buildingPos)}catch{setTownMessage("The old campaign record was damaged; a fresh ledger has been opened.")}}},[]);
  useEffect(()=>{localStorage.setItem("emberfall-save",JSON.stringify({rescued,blueprint,forgeLevel,gold,upgraded,maraGift,sanctumLevel,sanctumBlessing,hallLevel,buildingPos,difficulty,campaignWins,wandererRescued}))},[rescued,blueprint,forgeLevel,gold,upgraded,maraGift,sanctumLevel,sanctumBlessing,hallLevel,buildingPos,difficulty,campaignWins,wandererRescued]);
  useEffect(()=>{const prefs=localStorage.getItem("emberfall-settings");if(prefs){try{const p=JSON.parse(prefs);setMusicVolume(p.musicVolume??35);setEffectsVolume(p.effectsVolume??70);setScreenShake(p.screenShake??true);setLargeText(!!p.largeText);setHighContrast(!!p.highContrast)}catch{localStorage.removeItem("emberfall-settings")}}},[]);
  useEffect(()=>{localStorage.setItem("emberfall-settings",JSON.stringify({musicVolume,effectsVolume,screenShake,largeText,highContrast}));if(audioRef.current)audioRef.current.volume=musicVolume/100;document.body.classList.toggle("large-text",largeText);document.body.classList.toggle("high-contrast",highContrast)},[musicVolume,effectsVolume,screenShake,largeText,highContrast]);
  ...
  function preview(card:Card){
    if(!card.element)return card.damage?`${((card.damage+(upgraded.includes(card.id)?3:0))*(card.hits??1))} damage`:card.block?`${card.block+(upgraded.includes(card.id)?3:0)} Guard`:"Utility";
    const partner=(Object.keys(marks) as Element[]).find(e=>e!==card.element&&marks[e]>0);if(!partner)return `Applies ${card.element} mark`;
    const pair=[card.element,partner].sort().join("+");if(pair==="fire+water")return "STEAM - weaken next attack";if(pair==="fire+lightning")return `OVERLOAD - +${9+(relics.includes("Ember Lens")?3:0)} damage`;return "CONDUCT - +6 and chain";
  }
  ...
  function startCombat(index = 0, dangerous = false) {
    ...
    setDiscard([]);
    setExhausted([]);
    const escort=dangerous&&mapStep<3?FOES[(index+1)%FOES.length]:null;setSecondFoe(escort);setSecondHp(escort?Math.ceil(escort.hp*.65):0);setTargetSlot(0);
  ...
  function finishNode(){setMapStep(v=>v+1);setNodeType(null)}
  ...
  function claimReward(card?:Card){if(card)setRunDeck(v=>[...(v.length?v:deck),card]);setGold(v=>v+(elite?20:10));if(elite){setRelicChoice(shuffle(RELICS.filter(r=>!relics.includes(r.name))).slice(0,2));return}setMapStep(v=>v+1);setScreen("map")}
  function chooseRelic(name:string){setRelics(v=>[...v,name]);setRelicChoice(null);setMapStep(v=>v+1);setScreen("map")}
  function reforge(card:Card){if(gold<25||upgraded.includes(card.id))return;setGold(v=>v-25);setUpgraded(v=>[...v,card.id]);setForgeLevel(v=>Math.min(3,v+1));setTownMessage(`${card.name} is reforged: damage or Guard improves by 3.`)}
  ...
  function playCard(card: Card) {
    ...
    let dealt = ((card.damage ?? 0) + (card.damage && isUpgraded ? 3 : 0)) * (card.hits??1);
    ...
    if (dealt) {
      ...
      if(targetSlot===0){setEnemyArmor((value) => Math.max(0, value - dealt));setEnemyHp(Math.max(0, nextHp));if(mapStep>=3&&bossPhase===1&&nextHp>0&&nextHp<=Math.floor(foeMaxHp/2)){setBossPhase(2);setEnemyArmor(v=>v+12);setLog("The Warden tears free of its chains and raises a 12-Armor siege ward.")}}else{setSecondHp(Math.max(0,nextHp));if(nextHp<=0)setTargetSlot(0)}
      ...
      if (nextHp <= 0) {
        ...
        if (mapStep >= 3) { setRescued(true); setBlueprint(true); setCampaignWins(v=>v+1); window.setTimeout(()=>setScreen("victory"),650); }
  ...
  function endTurn() {
    ...
    if (intent.damage) {
      const scaled=intent.damage+(activeFoe.name==="Ash Hound"?Math.floor((turn-1)/2)*2:0)+(mapStep>=3&&bossPhase===2?3:0);
    ...
    const drawCount=Math.max(0,5-retained.length);setHand([...retained,...pile.slice(0,drawCount)]);
    ...
    return (
      <main className={`town-screen ${blueprint?"reclaimed":"ruined"}`}><audio ref={audioRef} src="/audio/dark-place.ogg" loop preload="metadata"/>
        <header className="topbar">
          ...
          <div className="resources"><button onClick={()=>setShowDeck(true)}>Deck</button><button onClick={()=>setShowRoster(true)}>Bestiary</button><button onClick={()=>setShowRules(true)}>Rules</button><button onClick={()=>setShowSettings(true)}>Settings</button><span>Blueprints <b>{blueprint ? 1 : 0}</b></span><span>Survivors <b>{rescued ? 1 : 0}</b></span></div>
        </header>
        ...
        <section className={`town-grid ${townEdit?"edit-mode":""}`} aria-label="Walkable overhead town" onClick={(event)=>{const rect=event.currentTarget.getBoundingClientRect();const pos={x:Math.max(2,Math.min(78,((event.clientX-rect.left)/rect.width)*100)),y:Math.max(3,Math.min(72,((event.clientY-rect.top)/rect.height)*100))};if(townEdit&&movingBuilding){setBuildingPos(v=>({...v,[movingBuilding]:pos}));setMovingBuilding(null);setTownMessage("Building placed. The new street plan is saved.")}else{setTownPos(pos);setTownMessage("Boots ring across the wet stones.")}}}>
        ...
        <aside className="town-panel">
          ...
          <div className="town-dialogue">{townMessage}</div><div className="gold-readout">Gold <b>{gold}</b>  Forge <b>{forgeLevel}/3</b>  Sanctum <b>{sanctumLevel}/3</b>  Hall <b>{hallLevel}/3</b></div>{campaignWins>0&&<label className="difficulty-select">Oath level <select value={difficulty} onChange={e=>setDifficulty(+e.target.value)}>{[0,1,2,3].slice(0,Math.min(4,campaignWins+1)).map(level=><option key={level} value={level}>{level} {level?` enemies +${level*12}% health`:" standard"}</option>)}</select></label>}<button className="town-edit" onClick={()=>{setTownEdit(v=>!v);setMovingBuilding(null)}}>{townEdit?"Finish city plan":"Arrange buildings"}</button><button className="primary" onClick={() => {setMapStep(0);setRunDeck([]);setScreen("attune")}}>Prepare expedition <span></span></button>
        </aside>
        {forgeOpen&&<div className="modal-backdrop" onClick={()=>setForgeOpen(false)}><section className="forge-modal" onClick={e=>e.stopPropagation()}><button className="modal-close" onClick={()=>setForgeOpen(false)}>Close</button><p className="eyebrow">MARA&apos;S FORGE</p><h2>Choose a card to reforge.</h2><p>Each reforging costs 25 gold and adds 3 damage or Guard.</p><div className="forge-list">{(runDeck.length?runDeck:deck).filter((card,index,array)=>array.findIndex(c=>c.id===card.id)===index).map(card=><button key={card.id} disabled={gold<25||upgraded.includes(card.id)} onClick={()=>reforge(card)}><b>{card.name}{upgraded.includes(card.id)?" +":""}</b><small>{upgraded.includes(card.id)?"Already reforged":card.text}</small></button>)}</div></section></div>}\n        {deckOverlay}
        {showRules&&<div className="modal-backdrop" onClick={()=>setShowRules(false)}><section className="rules-modal" onClick={e=>e.stopPropagation()}><button className="modal-close" onClick={()=>setShowRules(false)}>Close</button><p className="eyebrow">FIELD MANUAL</p><h2>Elemental reactions</h2><div className="reaction-rules"><article className="steam"><b>Fire + Water</b><h3>Steam</h3><p>Weakens the next enemy attack by 4 damage.</p></article><article className="conduct"><b>Water + Lightning</b><h3>Conduct</h3><p>Deals 6 bonus damage and chains to an escort.</p></article><article className="overload"><b>Fire + Lightning</b><h3>Overload</h3><p>Deals 9 bonus damage through enemy pressure.</p></article></div><h3>Combat glossary</h3><dl><dt>Retain</dt><dd>Stays in hand at end of turn.</dd><dt>Exhaust</dt><dd>Removed until the battle ends.</dd><dt>Burn</dt><dd>Deals damage at turn end, then decreases by one.</dd><dt>Vulnerable</dt><dd>Increases incoming damage by 50%.</dd></dl><small>Keyboard: 1-5 play cards  E ends turn  Escape pauses</small></section></div>}{showRoster&&<div className="modal-backdrop" onClick={()=>setShowRoster(false)}><section className="codex-modal" onClick={e=>e.stopPropagation()}><button className="modal-close" onClick={()=>setShowRoster(false)}>Close</button><p className="eyebrow">LEGION BESTIARY</p><h2>Enemies of the Ashen March</h2><div className="roster-grid">{[...FOES,...BOSSES].map((foe,index)=><article key={foe.name} className={index>=FOES.length?"elite-entry":""}><img src={`/art/${foe.art}.webp`} alt={foe.name}/><b>{foe.name}</b><small>{index>=FOES.length?"BOSS":"ENEMY"}</small>{"passive" in foe&&<p>{foe.passive}</p>}</article>)}</div></section></div>}
  ...
  return (
    <main className={`combat-screen ${screenShake&&impact?"shake":""}`}>
      ...
      <section className="battlefield">
        ...
        {secondFoe&&secondHp>0&&<button className={`escort-target ${targetSlot===1?"selected-target":""}`} onClick={()=>setTargetSlot(1)} aria-label={`Target ${secondFoe.name}`}><small>ESCORT INTENT  Attack 6</small><img src={`/art/${secondFoe.art}.webp`} alt={secondFoe.name}/><b>{secondFoe.name}</b><span>{secondHp}/{Math.ceil(secondFoe.hp*.65)}</span></button>}
        <div className="combatant demon">
          <div className="intent-card"><small>{mapStep>=3?`BOSS PHASE ${bossPhase}`:"NEXT INTENT"}</small><b>{intent.name}</b><span>{intent.damage?`Attacks for ${intent.damage+(activeFoe.name==="Ash Hound"?Math.floor((turn-1)/2)*2:0)+(mapStep>=3&&bossPhase===2?3:0)}`:intent.detail}{weakened&&intent.damage?"  weakened by 4":""}</span></div>
      ...
      <section className="hand-zone">
        ...
        <div className="hand">{hand.map((card,index) => <button key={card.id} disabled={card.cost > energy} className={`card ${card.element ?? "steel"} ${card.rarity||"common"}`} onClick={() => playCard(card)}><span className="cost">{card.cost}</span><kbd>{index+1}</kbd><small>{card.element ?? "KNIGHT"}{upgraded.includes(card.id)?"  UPGRADED":""}</small><h3>{card.name}{upgraded.includes(card.id)?" +":""}</h3><div className={`card-art ${card.element ?? "steel"}`}><span>{card.element ? "" : ""}</span></div><p>{card.text}</p><em className="card-preview">{preview(card)}</em></button>)}</div>
        <button className="end-turn" onClick={endTurn}>End turn <span></span></button>
      </section>
      <footer className="combat-footer"><button onClick={()=>setPileView("draw")}>Draw pile <b>{drawPile.length}</b></button><button onClick={()=>setPileView("discard")}>Discard <b>{discard.length}</b></button><button onClick={()=>setPileView("exhausted")}>Exhausted <b>{exhausted.length}</b></button><span>Reactions: <b>Steam Â· Conduct Â· Overload</b></span></footer>{deckOverlay}{pileOverlay}
